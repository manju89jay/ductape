"""Built-in C++ code generation (fallback when no pluggable emitter is available)."""
...
def generate_data_type_header(dt, sentinel, output_dir, registry):
...
def generate_converter(dt, config, output_dir, warning_module=None):
...
def generate_factory(data_types, output_dir):
...
def generate_field_provenance(registry, output_dir):
...
def generate_version_overview(registry, output_dir):
...
def copy_platform_types(config, output_dir):
